...
## 2026-09-17 — home frontend: random + dedupeByGroup

- Carga inicial: `getPropertiesRandom(30,{operation:'Venta'})` + `dedupeByGroup` local.
- Elimina N+1 de `getListingGroup` en el useEffect del home.
- `getPropertiesDeduped` / `getListingGroup` no borrados.
- Deep link `/?property=` sigue con `getProperties` completo.
