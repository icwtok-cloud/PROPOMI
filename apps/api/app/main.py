...
# devolverlo ÔÇö nunca se persiste texto sin sanitizar.
# --------------------------------------------------------------------------
_LEAK_PATTERNS = [
...
def sanitize_free_text(texto: str | None, campo: str = "comentario") -> str | None:
    """Devuelve el texto tal cual si est+í limpio. Si detecta un posible dato de
    contacto, rechaza con un error claro en vez de guardarlo silenciosamente
    filtrado ÔÇö coherente con la pol+¡tica definida para el crawler (doc 05):
...
def strip_contact_leaks(texto: str | None) -> str:
    ...
    desde el portal de origen no tiene a qui+®n devolverle un error ÔÇö se limpia
...
def validate_email_format(value: str | None) -> str | None:
    ...
    if not _EMAIL_RE.match(value):
        raise HTTPException(status_code=400, detail="Ingres+í un email v+ílido.")
    return value
...
JWT_ALGORITHM = "HS256"
# Etapa 4 (secci+¦n 10 / doc 6.2 ÔÇö panel de revisi+¦n manual de agencias):
# clave fija +¦nica, no por-usuario. Es intencionalmente simple (m+¡nimo
# viable, seg+¦n el plan maestro) ÔÇö no reemplaza un sistema de roles de
# equipo interno, que queda para m+ís adelante si hace falta.
...
class _RateLimiter:
    ...
    def check(self, key: str, limit: int, window: int = _RATE_WINDOW_SEC) -> None:
        ...
        while q and q[0] < now - window:
            q.popleft()
        if len(q) >= limit:
            raise HTTPException(status_code=429, detail="Demasiados intentos. Prob+í m+ís tarde.")
        q.append(now)
...
def _crawl_run_upsert(run_id: str, **fields: Any) -> dict[str, Any]:
    with _CRAWL_RUNS_LOCK:
        ...
        # LRU simple: borrar los m+ís viejos si hay demasiados
...
PROPERTY_FRESHNESS_DAYS = 60  # doc 05 (Etapa 2): filtro de cold-start ÔÇö una propiedad
# que el crawler no vuelve a ver hace m+ís de 60 d+¡as se considera potencialmente
...
# Services), por eso es seguro tenerlo como default ac+í ÔÇö pero en producci+¦n
...
class Property(Base):
    ...
    # DEPRECATED: texto libre que mezclaba "hace cu+ínto la detectamos" con
    # "lo que el portal de origen declara". Se mantiene solo por compatibilidad
    # de datos viejos ÔÇö usar detected_at (ya exist+¡a) + origin_published_at
    ...
    # hace 3 d+¡as") ÔÇö se preserva sin reinterpretar, separada de detected_at
    # (que es cu+índo el crawler/seed de Propomi la vio por primera vez).
    origin_published_at: Mapped[str | None] = mapped_column(String(100), nullable=True)
    source: Mapped[str] = mapped_column(String(160))
    source_url: Mapped[str] = mapped_column(String(500), default="#")
    # DEPRECATED: una sola imagen. Se mantiene por compatibilidad hacia atr+ís
    # (frontends viejos que todav+¡a lean `image`) ÔÇö el dato real y de uso
    # nuevo es `images` (lista JSON de hasta 5 URLs). ensure_schema_columns
    # migra autom+íticamente image -> images=[image] en filas viejas.
    ...
    # de auto-fusionarse o auto-descartarse ÔÇö decisi+¦n expl+¡cita de no
    ...
    # misma agencia), no es un dato sucio a revisar ÔÇö es la misma propiedad
...
class Agency(Base):
    ...
    city: Mapped[str] = mapped_column(String(100))
    # DEPRECATED en favor de verification_status ÔÇö se mantiene por compatibilidad
    ...
    # "claimeada" empieza en PENDING: ya tiene dashboard b+ísico (ve que tiene
    ...
    # verificarse pasa primero (SLA 24hs para ese caso). Mayor = m+ís prioridad.
...
class GeoCity(Base):
    """Ciudades agregadas por agentes (extienden el cat+ílogo est+ítico de geo_catalog.py)."""
...
class Subscription(Base):
    """Plan activo por agencia ÔÇö entidad propia (antes columnas en Agency)."""
...
class DemandRequest(Base):
    ...
    para la agencia due+¦a de la demanda ÔÇö canal agente-a-agente, siempre
...
class User(Base):
    ...
    # oferta". Ninguno de los dos reemplaza al otro ÔÇö el tel+®fono sigue
...
class Offer(Base):
    ...
    # Contacto real del comprador ÔÇö es lo que efectivamente se "revela" al
    # agente. Nunca se devuelve en ning+¦n endpoint hasta que exista un reveal
    # v+ílido (ver /offers/{id}/reveal).
    ...
    # slug de agencia). Nunca texto libre del comprador ÔÇö solo un c+¦digo corto
...
class Lead(Base):
    ...
    wizard que la oferta, pero sin forzar un monto ÔÇö a diferencia de Offer,
    ...
    # "Todav+¡a no tengo una propuesta" ÔÇö si la toc+¦, has_proposal=False y
...
class RevealTransaction(Base):
    ...
    Stripe, etc.) ÔÇö ver PaymentGateway m+ís abajo."""
...
class ColdStartTask(Base):
    ...
    principio. El tel+®fono scrapeado NUNCA viaja en endpoints p+¦blicos ÔÇö
    ...
    # Tel+®fono scrapeado / de la agencia no reclamada ÔÇö solo para el equipo
...
class AgentSuppressionList(Base):
    """Lista de baja permanente ÔÇö un tel+®fono/email ac+í nunca vuelve a
...
def ensure_schema_columns() -> None:
    required = {
        "properties": {
            ...
            # Etapa 1: im+ígenes m+¦ltiples + fecha de origen separada de
...
def ensure_schema_indexes() -> None:
    """+ìndices parciales que aceleran el listado home / random / freshness.
...
def migrate_legacy_property_images() -> None:
    ...
    arrancar de cero. Toda fila que todav+¡a tenga `images` vac+ío pero s+¡
    tenga el `image` (string) viejo, pasa a `images=[image]`. Es idempotente:
    una vez migrada, `images` deja de estar vac+ío y no se vuelve a tocar."""
    with Session(engine) as db:
...
def get_available_credit(db: Session, agency_id: str) -> int:
    """+Ünico punto de verdad del cupo de reveal. Fallback a columnas DEPRECATED si no hay filas nuevas."""
    total = 0
...
class VonageSmsSender:
    def send(self, phone: str, code: str) -> None:
        try:
            ...
            # llegar al cliente como error ÔÇö nunca como "ok:true" silencioso.
...
class PaymentGateway(Protocol):
    ...
    def create_checkout(self, agency_id: str, amount_usd: float, reference: str) -> str | None:
        ...
        (ej. el mock) ÔÇö en ese caso el 402 de /offers/{id}/reveal no incluye
        checkout_url."""
        ...
...
class MockPaymentGateway:
    ...
    sola ÔÇö devuelve False siempre, para que jam+ís se revele un contacto
...
# como Merchant of Record ÔÇö cobra la tarjeta +®l mismo con un checkout
...
# Reveal (pay-per-lead) ÔÇö compat con LEMON_SQUEEZY_VARIANT_ID legacy
...
class LemonSqueezyPaymentGateway:
    ...
    mismo request) ÔÇö la confirmaci+¦n real llega por
    POST /payments/webhooks/lemonsqueezy y de ahÔÇö se completa la
    ...
    def create_checkout(self, agency_id: str, amount_usd: float, reference: str) -> str | None:
        if not (LEMON_SQUEEZY_API_KEY and LEMON_SQUEEZY_STORE_ID and LEMON_SQUEEZY_VARIANT_ID):
            print("[PROPOMI LEMON SQUEEZY] Faltan variables de entorno (API_KEY/STORE_ID/VARIANT_ID) ÔÇö no se puede crear el checkout.")
            return None
        payload = {
            "data": {
                "type": "checkouts",
...
def _select_payment_gateway() -> PaymentGateway:
    ...
    if ENV == "production":
        ...
        print("[PROPOMI LEMON SQUEEZY] ENV=production sin credenciales configuradas ÔÇö usando MockPaymentGateway (todo pay-per-lead quedar+í 402 sin checkout_url).")
...
def phone_equivalents(phone: str) -> list[str]:
    ...
    seg+¦n qui+®n lo tipee (el 9 es solo el prefijo de m+¦vil): es la MISMA l+¡nea.
    La persona no tiene por qu+® saber con cu+íl se registr+¦, as+¡ que toda
    b+¦squeda por tel+®fono compara contra ambas variantes. El primer elemento
    ...
    if not rest.isdigit():
        return [phone]
    # Nacional AR = c+¦digo de +írea + n+¦mero = 10 d+¡gitos; con el 9 de m+¦vil, 11.
    # Ning+¦n c+¦digo de +írea argentino empieza con 9, no hay ambig++edad.
...
def slugify(name: str) -> str:
    ...
    No garantiza unicidad por s+¡ sola ÔÇö eso lo resuelve el caller agregando
...
def current_session(authorization: str | None = Header(default=None)) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=401, detail="Sesi+¦n inv+ílida o vencida") from exc
...
def current_user(session: dict[str, Any] = Depends(current_session)) -> User:
    with Session(engine) as db:
        ...
        if not user or user.phone != session.get("phone") or user.role != session.get("role") or user.agency_id != session.get("agency_id"):
            raise HTTPException(status_code=401, detail="Sesi+¦n inv+ílida")
        return user



def ensure_admin_seed() -> None:
    """Inserta el AdminUser +Ünico desde env (idempotente). Sin auto-registro API."""
...
def require_admin(
    ...
    if authorization and authorization.startswith("Bearer "):
        try:
            ...
            raise HTTPException(status_code=401, detail="Sesi+¦n admin inv+ílida o vencida") from exc
...
# --------------------------------------------------------------------------
# Etapa 2 (doc 05): ingesta del crawler. Un +Ünico endpoint recibe tanto altas
# nuevas como "el crawler volviÔÇö a ver esta misma publicaci+¦n" (upsert por
...
# verificaci+¦n ÔÇö no es p+¦blico, lo llama +¦nicamente el proceso del crawler.
# --------------------------------------------------------------------------
...
def find_possible_duplicate(db: Session, zone: str, price: float, surface: float, exclude_id: str | None = None) -> Property | None:
    ...
    zona en esta etapa es chico y as+¡ queda f+ícil de leer/ajustar tolerancias."""
...
def ingest_property(payload: PropertyIngestIn, _: None = Depends(require_admin)):
    ...
    with Session(engine) as db:
        ...
        if existing:
            ...
            # filtro de frescura de PROPERTY_FRESHNESS_DAYS) ÔÇö `detected_at`
            # NUNCA se toca ac+í, es la fecha de la primera vez que la vimos.
        ...
        # sobre la misma propiedad real (plan maestro 6.1) ÔÇö se agrupa por
...
ALLOWED_EVENTS = {
    ...
    # comprador. Se loguea autom+íticamente desde GET /properties (ver m+ís
    # abajo) y tambi+®n queda permitido ac+í por si el frontend alguna vez
    # necesita loguearlo manual v+¡a POST /events.
    "search_performed",
    # Etapa demanda gen+®rica B2B: se crea autom+íticamente desde el matching
...
class OfferIn(BaseModel):
    ...
    # Contacto real ÔÇö obligatorio: sin esto no hay nada que revelar despu+®s.
    ...
    # T9.3: c+¦digo de canal (solo [a-z0-9_-], m+íx 80). No es texto libre.
    ...
    def check_origin(cls, v: str | None) -> str | None:
        ...
        if not re.fullmatch(r"[a-z0-9][a-z0-9_-]{0,79}", v):
            raise HTTPException(status_code=400, detail="Origen inv+ílido.")
        return v
...
class AgencyUpdate(BaseModel):
    ...
    # 06.2.8), pero ac+í solo se captura el dato ÔÇö el pasaje a VERIFIED lo hace
...
def register_agency(payload: AgencyRegisterIn, request: Request):
    ...
    403 si el tel+®fono no est+í asociado a ninguna Agency. Este endpoint solo
    crea el registro de Agency+tel+®fono; el login sigue siendo el mismo
    flujo de OTP de siempre (request ÔåÆ verify), sin contrase+¦a.

    Queda en verification_status=PENDING igual que cualquier otra agencia
    reci+®n claimeada ÔÇö dashboard b+ísico, sin poder revelar contactos ni
    ...
    if not phone:
        raise HTTPException(status_code=400, detail="Ingres+í un tel+®fono v+ílido.")
    name = payload.name.strip()
    city = payload.city.strip()
    if not name or not city:
        raise HTTPException(status_code=400, detail="Complet+í nombre de la agencia y ciudad.")
    with Session(engine) as db:
        if find_agency_by_phone(db, phone):
            raise HTTPException(
                status_code=409,
                detail="Ya existe una agencia registrada con este tel+®fono. Inici+í sesi+¦n normalmente pidiendo el c+¦digo.",
            )
        agency = Agency(
...
def request_otp(payload: OTPRequest, request: Request):
    ...
    with Session(engine) as db:
        ...
        if recent_count >= OTP_MAX_REQUESTS:
            raise HTTPException(status_code=429, detail="Alcanzaste el l+¡mite de solicitudes. Prob+í nuevamente m+ís tarde.")
...
def consume_valid_otp(db: Session, phone: str, code: str) -> None:
    ...
    HTTPException si el c+¦digo es inv+ílido/vencido/excedido en intentos.
    ...
    rate limit ÔÇö el `OTPRequest`/generaci+¦n del c+¦digo sigue siendo el mismo
    para ambos roles, solo cambia qu+® se hace DESPU+ëS de validar el c+¦digo."""
    ...
    if otp.verify_attempts >= OTP_MAX_VERIFY_ATTEMPTS:
        otp.consumed = True
        db.commit()
        raise HTTPException(status_code=429, detail="Demasiados intentos. Solicit+í un nuevo c+¦digo m+ís tarde.")
...
def verify_otp(payload: OTPVerify, request: Request):
    ...
    if not phone:
        ...
        raise HTTPException(status_code=400, detail="Tel+®fono inv+ílido")
    with Session(engine) as db:
        ...
        if user is None:
            if not agency:
                raise HTTPException(
                    status_code=403,
                    detail="No encontramos una agencia asociada a este tel+®fono. Si todav+¡a no te registraste, volv+® atr+ís y toc+í -½-+Reci+®n arranc+ís? Cre+í tu agencia-+.",
                )
...
def verify_otp_buyer(payload: OTPVerify, request: Request):
    ...
    mismo hash, mismo rate limit, mismo TTL ÔÇö ver `consume_valid_otp`), pero
    sin exigir que el tel+®fono est+í asociado a una agencia (a diferencia de
    ...
    with Session(engine) as db:
        ...
        # (el tel+®fono es la identidad can+¦nica, secci+¦n 5) ÔÇö no se lo
...
def link_google_identity(payload: GoogleAuthIn, request: Request, session: dict[str, Any] = Depends(current_session)):
    ...
    pedida reci+®n en el +¦ltimo paso del wizard de oferta, ADEM+üS del celular
    verificado por OTP (nunca en su lugar). Requiere una sesi+¦n ya vigente
    (guest o comprador con celular verificado) ÔÇö este endpoint solo VINCULA
    ...
    try:
        ...
        # Token mal formado, firma inv+ílida, audience distinto o vencido ÔÇö
        # esto s+¡ es responsabilidad del cliente, 401.
        raise HTTPException(status_code=401, detail="Token de Google inv+ílido o vencido") from exc
    except google_auth_exceptions.TransportError as exc:
        # No se pudo llegar a googleapis.com para bajar las claves p+¦blicas
        # de verificaci+¦n ÔÇö es un problema de red transitorio, no un token
        # inv+ílido. 401 ser+¡a enga+¦oso ac+í (el usuario reintentar+¡a con el
        # mismo bot+¦n y volver+¡a a fallar por la misma raz+¦n de red).
        raise HTTPException(status_code=503, detail="No pudimos verificar con Google en este momento. Prob+í de nuevo en unos segundos.") from exc
    ...
    with Session(engine) as db:
        ...
        if not user.phone_verified_at:
            raise HTTPException(status_code=403, detail="Verific+í tu celular antes de vincular Google.")
        other = db.scalar(select(User).where(User.google_sub == google_sub, User.id != user.id))
        if other:
            raise HTTPException(status_code=409, detail="Esta cuenta de Google ya est+í vinculada a otro usuario de Propomi.")
...
def find_agency_by_phone(db: Session, phone: str) -> Agency | None:
    ...
    que entrar con 11 XXXX XXXX, +54 11 XXXX XXXX o +54 9 11 XXXX XXXX d+® el
    mismo resultado sin importar con cu+íl se dio de alta la agencia."""
...
def agency_login_phone(db: Session, agency: Agency, phone: str) -> str:
    """El tel+®fono con el que la agencia YA est+í guardada y que equivale a
...
def find_user_by_phone(db: Session, phone: str) -> User | None:
    """Busca un usuario por tel+®fono exacto o por su variante con/sin el 9 de
    m+¦vil argentino. Si hubiera m+ís de una fila (formatos distintos creados en
...
def _bulk_group_info(db: Session, props: list) -> dict[str, dict]:
    """Una sola query agregada por listing_group_id ÔåÆ evita N+1 del home.
...
def get_listing_group(property_id: str):
    """Etapa 019 (plan maestro 6.1): si esta propiedad est+í agrupada porque
    ...
    `grouped: false` en vez de 404 ÔÇö no tener grupo es el caso normal, no un
    error."""
    with Session(engine) as db:
...
def review_queue(_: None = Depends(require_admin)):
    ...
    `POST /properties/ingest` marcÔÇö como posible duplicado. Devuelve cada
...
def resolve_review(property_id: str, payload: ReviewResolutionIn, _: None = Depends(require_admin)):
    ...
    - "confirm_duplicate": es realmente el mismo aviso duplicado -> se oculta
      ...
      circulaci+¦n baj+índola de la b+¦squeda p+¦blica v+¡a `last_seen_at` muy
...
# Heur+¡stica de ciudad v+ílida para el selector (presentaci+¦n +¦nicamente; no
...
#   5. contiene espacios y termina en altura num+®rica de 3ÔÇô5 d+¡gitos
_CITY_ADDR_PREFIX = re.compile(
    r"^(av\.?|avenida|calle|ruta|pasaje|pje\.?)\b",
    re.IGNORECASE,
)
_CITY_HAS_LETTER = re.compile(r"[A-Za-z+ü+ë+ì+ô+Ü+£+æ+í+®+¡+¦+¦+++¦]")
...
def _normalize_city_name(s: str) -> str:
    """NFKD + ascii + lower + strip ÔÇö misma idea que el backfill de province."""
...
@app.get("/geo/catalog")
def geo_catalog():
    """Cat+ílogo geogr+ífico (est+ítico + ciudades agregadas por agentes) para el
...
def add_geo_city(payload: GeoCityIn, session: dict[str, Any] = Depends(require_agent)):
    ...
    city = payload.city.strip()
    if not city:
        raise HTTPException(status_code=400, detail="Ingres+í el nombre de la ciudad")
    if country not in GEO_CATALOG or province not in GEO_CATALOG.get(country, {}):
        raise HTTPException(status_code=400, detail="Pa+¡s o provincia no v+ílidos en el cat+ílogo")
    ...
    # Universo can+¦nico: est+íticas + DB
    with Session(engine) as db:
        db_rows = db.scalars(
...
def properties(
    ...
    # una sesi+¦n v+ílida (agente o comprador) se guarda el user_id para
    # an+ílisis de demanda, igual que en cualquier otro evento del sistema.
    ...
    # include_hidden=true solo con X-Admin-Key v+ílida (un agente no ve ocultas de otros).
    include_hidden: bool = False,
    x_admin_key: str | None = Header(default=None, alias="X-Admin-Key"),
):
    session = None
    if authorization:
        try:
            ...
            # Token vencido/ inv+ílido en una b+¦squeda no debe romper la
            # b+¦squeda en s+¡ ÔÇö solo se pierde la asociaci+¦n a un user_id.
            session = None
    ...
    with Session(engine) as db:
        ...
        # Etapa 3: evento agregado y an+¦nimo por cada b+¦squeda ÔÇö insumo para
...
def properties_random(
    ...
    n default 30, m+íximo 60 (clamp silencioso). Misma pol+¡tica de hidden/frescura
...
def create_property(payload: PropertyCreateIn, session: dict[str, Any] = Depends(require_agent)):
    ...
    with Session(engine) as db:
        ...
        if agency.verification_status != "VERIFIED":
            raise HTTPException(
                status_code=403,
                detail="Tu agencia todav+¡a no est+í verificada. Solo agencias verificadas pueden cargar propiedades.",
            )
...
def create_demand_request(payload: DemandRequestIn, session: dict[str, Any] = Depends(require_agent)):
    ...
    con PLAN_50 o PLAN_99 (>= $60/mes) pueden publicar qu+® est+ín buscando para
    ...
    with Session(engine) as db:
        ...
        if plan not in DEMAND_REQUEST_ALLOWED_PLANS:
            raise HTTPException(
                status_code=403,
                detail="Esta funci+¦n requiere plan PLAN_50 o PLAN_99. Actualiz+í tu plan desde Mi cuenta.",
            )
        ...
        if active_count >= DEMAND_REQUEST_MAX_ACTIVE_PER_AGENCY:
            raise HTTPException(status_code=400, detail=f"Llegaste al m+íximo de {DEMAND_REQUEST_MAX_ACTIVE_PER_AGENCY} demandas activas.")
...
def create_event(payload: EventIn, authorization: str | None = Header(default=None)):
    ...
    if payload.name not in ALLOWED_EVENTS:
        raise HTTPException(status_code=400, detail="Evento inv+ílido")
    with Session(engine) as db:
...
def create_offer(payload: OfferIn, session: dict[str, Any] = Depends(current_session)):
    ...
    if not buyer_phone_normalized:
        raise HTTPException(status_code=400, detail="Ingres+í un tel+®fono de contacto v+ílido")
    with Session(engine) as db:
        # Etapa 2 / secci+¦n 6.2.1: enforcement real del lado del servidor ÔÇö
        ...
        if not buyer_user or not buyer_user.phone_verified_at:
            raise HTTPException(status_code=403, detail="Verific+í tu celular antes de enviar una oferta.")
...
def validate_visit_slot(visit_day: str | None, visit_slot: str | None) -> None:
    ...
    (hoy incluido) y visit_slot uno de los tres valores fijos ÔÇö mismo
    ...
    if visit_slot not in VALID_VISIT_SLOTS:
        raise HTTPException(status_code=400, detail="Franja horaria inv+ílida.")
    try:
        day = datetime.strptime(visit_day, "%Y-%m-%d").date()
    except ValueError:
        raise HTTPException(status_code=400, detail="Fecha de visita inv+ílida.")
...
def create_lead(payload: LeadIn, session: dict[str, Any] = Depends(current_session)):
    ...
    with Session(engine) as db:
        buyer_user = db.get(User, session["user_id"])
        if not buyer_user or not buyer_user.phone_verified_at:
            raise HTTPException(status_code=403, detail="Verific+í tu celular antes de continuar.")
        p = db.get(Property, payload.property_id)
        if not p:
            raise HTTPException(status_code=404, detail="Propiedad no encontrada")
...
def list_offers(status: str | None = None, session: dict[str, Any] = Depends(current_session)):
    ...
    cantidad de ofertas esperando ÔÇö nunca monto, propiedad ni ning+¦n detalle.
    ...
    with Session(engine) as db:
        ...
        if session.get("role") == Role.AGENTE.value:
            ...
            if not agency or agency.verification_status != "VERIFIED":
                # Solo conteo ÔÇö sin ids, montos ni property_id (anti-fuga de detalle comercial
...
def counter_offer(offer_id: str, payload: CounterIn, session: dict[str, Any] = Depends(require_agent)):
    with Session(engine) as db:
        agency = db.get(Agency, session["agency_id"])
        if not agency or agency.verification_status != "VERIFIED":
            raise HTTPException(status_code=403, detail="Tu agencia todav+¡a no est+í verificada. No pod+®s responder ofertas hasta estar Verificada.")
...
def enforce_listing_group_reveal_priority(
    ...
    """T8.7: dentro de la ventana, solo la agencia con suscripci+¦n m+ís antigua
    ...
    if agency.id != oldest.id:
        raise HTTPException(
            status_code=403,
            detail=(
                f"Esta oferta est+í en una ficha multi-agente. Durante las primeras "
                f"{window_h}h tiene prioridad la agencia con la suscripci+¦n m+ís antigua. "
...
def reveal_contact(offer_id: str, session: dict[str, Any] = Depends(require_agent)):
    """
    +Ünico punto del sistema que puede exponer el nombre/tel+®fono real del
    comprador al agente. Reemplaza el flujo anterior (que revelaba el
    tel+®fono de la AGENCIA al comprador ÔÇö direcci+¦n invertida respecto del
    modelo de negocio documentado). Reglas:
      ...
      4. Si no hay cupo o no hay suscripci+¦n, se cobra pay-per-lead v+¡a
         ...
         pago ÔÇö nunca revela gratis por default.
    """
    with Session(engine) as db:
        offer = db.get(Offer, offer_id)
        if not offer:
        ...
        if agency.verification_status != "VERIFIED":
            raise HTTPException(
                status_code=403,
                detail="Tu agencia todav+¡a no est+í verificada. Complet+í Instagram/link de tu perfil y esper+í la revisi+¦n para poder revelar contactos.",
            )

        now = datetime.now(timezone.utc)
        enforce_listing_group_reveal_priority(db, prop, agency, offer, now)

        # Cupo +Ünico v+¡a get_available_credit / consume_reveal_credit.
        ...
        # transacci+¦n completada pendiente de aplicar (idempotencia b+ísica).
...
def reveal_lead_contact(lead_id: str, session: dict[str, Any] = Depends(require_agent)):
    ...
    pay-per-lead via PaymentGateway despues ÔÇö nunca revela gratis por
    default."""
    with Session(engine) as db:
...
def payment_status(transaction_id: str, session: dict[str, Any] = Depends(require_agent)):
    ...
    nueva si todav+¡a no se pag+¦). Nunca revela nada ac+í ÔÇö solo dice si el
    pago quedÔÇö COMPLETED; el reveal real sigue pasando por
...
if ENV != "production":
    ...
    def mock_complete_payment(transaction_id: str, session: dict[str, Any] = Depends(require_agent)):
        """SOLO disponible fuera de producci+¦n ÔÇö simula la confirmaci+¦n de una
...
def create_payment_checkout(payload: CheckoutRequestIn, session: dict[str, Any] = Depends(require_agent)):
    ...
    if not variant_id:
        raise HTTPException(status_code=400, detail="Plan inv+ílido o variant no configurado")
    if kind == "reveal":
        transaction_id = f"rt-{uuid.uuid4().hex[:12]}"
        with Session(engine) as db:
            db.add(RevealTransaction(
...
async def lemonsqueezy_webhook(
    ...
    """Webhook Lemon Squeezy ÔÇö firma HMAC obligatoria.
    ...
    if not hmac.compare_digest(expected, x_signature):
        raise HTTPException(status_code=401, detail="Firma inv+ílida")
...
def offer_action(offer_id: str, action: str, session: dict[str, Any] = Depends(require_agent)):
    if action not in {"accept", "reject", "negotiate"}: raise HTTPException(status_code=400, detail="Acci+¦n inv+ílida")
    with Session(engine) as db:
        agency = db.get(Agency, session["agency_id"])
        if not agency or agency.verification_status != "VERIFIED":
            raise HTTPException(status_code=403, detail="Tu agencia todav+¡a no est+í verificada. No pod+®s gestionar ofertas hasta estar Verificada.")
        offer = db.get(Offer, offer_id)
        if not offer: raise HTTPException(status_code=404, detail="Oferta no encontrada")
        prop = db.get(Property, offer.property_id)
        if not prop or prop.agency_id != session["agency_id"]: raise HTTPException(status_code=403, detail="Oferta fuera de tu agencia")
...
def contact_request(payload: ContactIn, session: dict[str, Any] = Depends(require_agent)):
    """
    Canal B2B (agente <-> agencia), gratuito e inmediato ÔÇö ver doc 03.
    El comprador NUNCA obtiene el contacto de la agencia por ac+í: su +Ünico
    ...
    with Session(engine) as db:
        ...
        if agency.phone:
            ...
            if suppressed:
                raise HTTPException(status_code=403, detail="Esta agencia solicit+¦ no recibir m+ís contactos en la plataforma.")
...
def agency_opt_out(agency_id: str, session: dict[str, Any] = Depends(require_agent)):
    ...
    dise+¦o ÔÇö quien quiera volver debe contactar soporte, no autoservicio."""
...
def agency_by_slug(slug: str):
    ...
    verificaci+¦n) ÔÇö nunca tel+®fono ni ning+¦n dato de contacto."""
    with Session(engine) as db:
        ensure_seed(db)
        ensure_agency_slugs(db)
...
def set_agency_subscription(agency_id: str, payload: SubscriptionIn, session: dict[str, Any] = Depends(require_agent)):
    ...
    if plan not in PLAN_CUPO:
        raise HTTPException(status_code=400, detail=f"Plan inv+ílido. Valores: {', '.join(PLAN_CUPO.keys())}")
    with Session(engine) as db:
        a = db.get(Agency, agency_id)
        if not a:
            raise HTTPException(status_code=404, detail="Agencia no encontrada")
...
def add_agency_phone(agency_id: str, payload: PhoneIn, session: dict[str, Any] = Depends(require_agent)):
    ...
    with Session(engine) as db:
        if not db.get(Agency, agency_id):
            raise HTTPException(status_code=404, detail="Agencia no encontrada")
        if find_agency_by_phone(db, phone):
            raise HTTPException(status_code=409, detail="Ese tel+®fono ya est+í asociado a una agencia")
...
class SearchPerformedIn(BaseModel):
    """Filtros an+¦nimos de b+¦squeda ÔÇö sin PII."""
...
async def post_search_performed(request: Request):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="JSON inv+ílido")
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Filtros inv+ílidos: {exc}") from exc
    zone = payload.zone
...
def demand(limit: int = 500, session: dict[str, Any] = Depends(require_agent)):
    ...
    los agrega en rankings simples de demanda (zonas y tipos m+ís buscados).
    No hay tabla propia de agregaci+¦n todav+¡a ÔÇö se calcula al vuelo sobre
...
def market_opportunities(
    ...
    contra las zonas donde ESTA agencia tiene cat+ílogo. Solo VERIFIED.
    ...
    with Session(engine) as db:
        ...
        if agency.verification_status != "VERIFIED":
            raise HTTPException(
                status_code=403,
                detail="Tu agencia todav+¡a no est+í verificada. Las oportunidades de mercado solo est+ín disponibles para cuentas Verificadas.",
            )

        props = db.scalars(select(Property).where(Property.agency_id == agency_id)).all()
        # Zonas del cat+ílogo de la agencia (solo esas ÔÇö no mostrar demanda de zonas que no vende)
        ...
        for zone, st in zone_stats.items():
            ...
            # rango m+ís buscado: mediana simple de max_price
            budget_median = None
            if prices:
                mid = len(prices) // 2
...
def supply_demand_suggestions(
    ...
    """Cruza demanda (search_performed agregados) con oferta del cat+ílogo
    ...
    with Session(engine) as db:
        ...
        return {
            ...
            "note": "Demanda an+¦nima global; oferta = cat+ílogo no oculto ordenado por priority_score.",
        }
...
def pricing_hint(
    ...
    """Rango de precio sugerido por mediana de cat+ílogo en zona (+filtros).
    ...
    with Session(engine) as db:
        ...
        if len(prices) < 3:
            return {
                ...
                "sample": len(prices),
                "hint": None,
                "message": "Muestra insuficiente (<3) en cat+ílogo para esa zona/filtros.",
            }
        mid = len(prices) // 2
        ...
        return {
            ...
            "currency": "USD",
            "method": "percentiles del cat+ílogo actual (misma base que priority_score)",
        }
...
def admin_auth_verify_otp(payload: AdminOtpVerifyIn, request: Request):
    """Paso 2: OTP v+ílido ? JWT role=ADMIN (8h)."""
    ensure_admin_seed()
    _rate.check(f"auth:ip:{_client_ip(request)}", RATE_AUTH_PER_IP)
    with Session(engine) as db:
        admin = db.scalars(select(AdminUser).where(AdminUser.username == payload.username.strip())).first()
        if not admin:
            raise HTTPException(status_code=401, detail="Usuario inv+ílido")
        now = datetime.now(timezone.utc)
        otp = db.scalar(
            select(OTPCode)
        ...
        if not otp:
            raise HTTPException(status_code=401, detail="C+¦digo OTP inv+ílido o vencido")
        exp = otp.expires_at
...
def admin_pending_agencies(_: None = Depends(require_admin)):
    ...
    por `verification_priority` descendente (doc 6.2 ÔÇö quien ya se
...
def admin_approve_agency(agency_id: str, request: Request, payload: AgencyReviewIn | None = None, _: None = Depends(require_admin)):
    with Session(engine) as db:
        ...
        a.verified = True  # DEPRECATED, se mantiene en sync por compatibilidad hacia atr+ís
        ...
        # Los 10 leads gratis al verificarse (doc 06.2.3/08) se otorgan ac+í,
        # una sola vez ÔÇö si por alg+¦n motivo ya ten+¡a cupo cargado (no
...
def admin_list_agencies(
    ...
    No expone datos de compradores ÔÇö solo perfil de agencia."""
    ...
    with Session(engine) as db:
        ...
        if status:
            ...
            if st not in {"PENDING", "VERIFIED", "REJECTED"}:
                raise HTTPException(status_code=400, detail="status inv+ílido")
...
def admin_cold_start_pending(_: None = Depends(require_admin)):
    """Cola de notificaciones manuales T6.1. +Ünico endpoint que expone el
    tel+®fono scrapeado de la agencia no reclamada ÔÇö protegido por X-Admin-Key.
        ...
        return [
            {
                ...
                "messageTemplate": (
                    f"Hola ÔÇö alguien ofreci+¦ {t.currency} {t.amount:,.0f} por "
                    f"-½{t.property_title}-+ ({t.property_zone}) en Propomi. "
                    f"Reclam+í tu perfil y ver el detalle: "
...
def get_onboarding(token: str):
    ...
    datos del comprador ÔÇö solo lo necesario para motivar el claim."""
    with Session(engine) as db:
        task = db.scalar(select(ColdStartTask).where(ColdStartTask.onboarding_token == token))
        if not task:
            raise HTTPException(status_code=404, detail="Link inv+ílido o vencido")
        ...
        if task.status == "CLAIMED":
            return {
                ...
                "message": "Este perfil ya fue reclamado. Entr+í a /agencia con tu tel+®fono.",
            }
...
def complete_onboarding(
    ...
    with Session(engine) as db:
        ...
        if session.get("agency_id") != task.agency_id:
            raise HTTPException(
                status_code=403,
                detail="Entr+í con el tel+®fono de la agencia asociada a esta invitaci+¦n.",
            )
        agency = db.get(Agency, task.agency_id)
        if not agency:
            raise HTTPException(status_code=404, detail="Agencia no encontrada")
        ...
        return {
            ...
            "message": "Perfil reclamado. Complet+í la verificaci+¦n desde el panel de agencia si todav+¡a est+í pendiente.",
        }
...
def admin_run_crawler(
    ...
    if not x_admin_key or not secrets.compare_digest(x_admin_key, ADMIN_KEY):
        raise HTTPException(status_code=401, detail="Admin key inv+ílida")
    _rate.check(f"admin:ip:{_client_ip(request)}", RATE_ADMIN_PER_IP)
    ...
    return {
        ...
        "crawl_run_id": run_id,
        "sources": source_ids,
        "message": "Crawl encolado. Consult+í GET /admin/crawler/status/{crawl_run_id}",
    }
...
def admin_crawler_status(
    ...
    """Progreso de una corrida (o la m+ís reciente si no se pasa id)."""
    if not x_admin_key or not secrets.compare_digest(x_admin_key, ADMIN_KEY):
        raise HTTPException(status_code=401, detail="Admin key inv+ílida")
    _rate.check(f"admin:ip:{_client_ip(request)}", RATE_ADMIN_PER_IP)
    with _CRAWL_RUNS_LOCK:
