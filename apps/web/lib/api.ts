...
export async function getAgency(id:string,session?:Session|null){if(!base)return {id,name:'Agencia demo',city:'Buenos Aires',verified:true,claimed:true,phone:'+5491155550101'} as Agency;return req<Agency>(`/agencies/${id}`,undefined,session?.token)}
export async function updateAgency(id:string,name:string,session:Session){if(!base)return {id,name,verified:true};return req(`/agencies/${id}`,{method:'PATCH',body:JSON.stringify({name})},session.token)}
